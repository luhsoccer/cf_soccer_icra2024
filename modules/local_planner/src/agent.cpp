...
std::vector<Marker> Agent::getMarkers(const std::shared_ptr<const transform::WorldModel>& wm,
    ...
    auto markers =
        this->skill->getVisualizationMarkers(wm, this->task_data.value(), this->robot, time, this->current_step);
    const double left_shift = static_cast<double>(skill->name.length()) / 2.0 * 0.05;
    marker::Text skill_text(
        {this->robot.getFrame(), -left_shift, SKILL_TEXT_OFFSET}, "", 0,
        fmt::format("{}({:d}/{:d})", this->skill->name, this->current_step, this->skill->steps.size()));
    skill_text.setHeight(SKILL_TEXT_HEIGHT);
    skill_text.setColor(marker::Color::WHITE());
    markers.emplace_back(skill_text);
    return markers;
}

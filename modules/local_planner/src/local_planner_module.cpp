...
LocalPlannerModule::LocalPlannerModule(const std::shared_ptr<const transform::WorldModel>& wm,
    ...
    : cs(config_provider::ConfigProvider::getConfigStore()),
      ...
      ms(ms),
      simulation_manager(this->thread_pool, ms, wm),
      avoidance_manager(std::make_shared<AvoidanceManager>(simulation_manager, wm, ms)),
      logger("local_planner_module") {
    this->simulation_manager.linkAvoidanceManager(avoidance_manager);
    marker::Info version("local planner", 0);
    std::string version_str("0.0.13");
    version.set("Version", version_str);
    ms.displayMarker(version);
}
...
bool LocalPlannerModule::setTask(const Skill* skill, const TaskData& td) {
    try {
        ...
        this->simulation_manager.setNewTask(td.robot, skill, td);
        return true;

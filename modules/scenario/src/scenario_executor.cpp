...
void ScenarioExecutor::executeScenario(std::atomic_bool& should_run, Scenario& scenario) {
    for (size_t i = 0; i < this->repetitions; i++) {
        ...
        time::Rate rate(100);
        while (should_run && !scenario.isFinished(this->local_planner_module, time::now() - start_time)) {
            this->exp_logger.newDataPoint();
            scenario.teleportRobots(wm, this->simulation_interface, this->local_planner_module, this->skill_book,

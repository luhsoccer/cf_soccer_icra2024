...
BallPlacement::BallPlacement()
    : DriveStepConstraint(),
      weight(CALLBACK, [this](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
          ...
          if (res.vec->norm() < localPlannerConfig().stop_state_min_ball_distance + config.robot_radius * 1) {
              return 0.0;
          } else {
              return 1.0;
          }
...
std::vector<std::shared_ptr<const AbstractTargetFeature>> BallPlacement::getAdditionalTargetFeaturesImpl() const {
    DoubleComponentParam anti_weight(
        CALLBACK, [this](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            ...
            auto robot_pos = wm->getTransform(td.robot.getFrame());
            const static LineShape LINE("ball", {this->BALL_PLACEMENT_POSITION_FRAME});
            ...
            const auto close_to_border = [&field_data, &config](Eigen::Affine2d position) -> bool {
                return std::abs(position.translation().x()) > field_data.size.x() / 2 + field_data.field_runoff_width -
                                                                  ...
                                                                  config.robot_radius * 3 ||
                       std::abs(position.translation().y()) > field_data.size.y() / 2 + field_data.field_runoff_width -
                                                                  config.stop_state_min_ball_distance -
                                                                  config.robot_radius * 3;
            };
            if (ball_pos.has_value() && robot_pos.has_value() && placement_pos.has_value() &&
                robot_to_line.vec.has_value() &&
                robot_to_line.vec->norm() < config.stop_state_min_ball_distance + config.robot_radius * 3 &&
                close_to_border(robot_pos->transform) &&
                (close_to_border(Eigen::Affine2d(
                    (Eigen::Translation2d(robot_pos->transform.translation() + robot_to_line.vec.value())) *
    ...
    // DoubleComponentParam anti_weight_ball(
    //     CALLBACK, [this](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
    //         const auto& config = localPlannerConfig();
    //         auto ball_pos = wm->getTransform("ball");
    //         auto placement_pos = wm->getTransform(this->BALL_PLACEMENT_POSITION_FRAME);
    ...
    //         }
    //     });
    DoubleComponentParam anti_k(
        CALLBACK, [anti_weight](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            if (anti_weight.val(wm, td) > 1.0) {
                return 20.0;
            } else {
                return 0.0;
            }
    ...
    DoubleComponentParam border_k(
        CALLBACK,
        [border_weight](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            if (border_weight.val(wm, td) > 1.0) {
                return 20.0;
            } else {
                return 0.0;
            }
                ...
                RectangleShape(
                    ...
                    {CALLBACK,
                     [](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& /*td*/) -> double {
                         ...
                         return field_data.size.y() + 2 * field_data.field_runoff_width + 0.3;
                     }},
                    {CALLBACK,
                     [](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& /*td*/) -> double {
                         auto field_data = wm->getFieldData();
                         return field_data.size.x() + 2 * field_data.field_runoff_width + 0.3;
                     }},
                    false),
                border_anti_target_influence_distance, border_weight, std::nullopt, std::nullopt, 100),
            std::make_shared<AntiTargetFeature>(PointShape("ball"), localPlannerConfig().stop_state_min_ball_distance,
                                                100, std::nullopt, std::nullopt, 100)};
}

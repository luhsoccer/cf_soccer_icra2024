    ...
    for (const auto other_robot : transform::RobotDataStorage::generateAllPossibleRobots(MAX_ROBOTS_PER_TEAM)) {
        DoubleComponentParam w(
            ...
            [other_robot](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
                // check if robot in defense area
                auto robot_pos = wm->getTransform(td.robot.getFrame());
                const auto field_data = wm->getFieldData();
                if (!robot_pos.has_value()) return 0.0;

                if (std::abs(robot_pos->transform.translation().x()) >
                        field_data.size.x() / 2 - field_data.penalty_area_depth &&
                    std::abs(robot_pos->transform.translation().y()) < field_data.penalty_area_width / 2) {
                    return 0.0;
                }

...
BodSkillBook::BodSkillBook(const config_provider::ConfigStore& cs)
    : SkillBook<BodSkillNames>({
          ...
          {BodSkillNames::RECEIVE_BALL_WITHOUT_CANCELLATION,ReceiveBallWithoutCancellationBuild().build(cs)},
          {BodSkillNames::GOALIE_INTERCEPT,GoalieInterceptBuild().build(cs)},
          {BodSkillNames::DO_NOT_INTERFERE_WITH_BALL_PLACEMENT,DoNotInterfereWithBallPlacementBuild().build(cs)},

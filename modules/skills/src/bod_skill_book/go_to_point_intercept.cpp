...
void GoToPointInterceptBuild::buildImpl(const config_provider::ConfigStore& cs) {
    ...
    pass_ball_up.addFeature(
        TargetFeature(LineShape(past_ball_pos, "ball", -10.0, cs.skills_config.move_to_ball_pre_radius)));
    pass_ball_up.addFeature(
        TargetFeature(LineShape(past_ball_pos, "ball", -10.0, 0.0), DEFAULT_TRANSLATIONAL_TOLERANCE, 0.001));
